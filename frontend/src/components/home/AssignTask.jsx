...
const AssignTask = ({ 
    ...
    const handleCreateTask = async () => {
        ...
        const { data: customerResponse, error: customerError } = await addCustomer(customerInfo);
        if (customerResponse) {
            ...
            const { data: taskResponse, error: taskError } =  await createTask(newTask);
            ...
            <div className="modal-dialog modal-dialog-centered">
                <div className="modal-content" >
                    ...
                    <div className="modal-body " style={{ height: "450px" }}>
                        ...
                        <form >
                            ...
                            <div className='assign-user'>
                                ...
                                <Container fluid className='p-0 my-3  overflow-auto' style={{ height: "350px" }}>
                                    {console.log(usersArr)}
                                    {usersArr.map((myUser, index) => (
                                        <div 
                                            className={`
                                                d-flex gap-4 ${selectedUser === myUser.id ? 'bg-light' : ''} align-items-center p-2 border-bottom
                                            `}
                                        ...
                                        >
                                            <input 
                                                ...
                                                value={myUser.id} 
                                                checked={selectedUser === myUser.id} 
                                                onChange={() => setSelectedUser(myUser.id)} 
                                            />
                                            <div>
                                                <div className='d-flex gap-2'>
                                                    <p>{myUser?.name}</p>
                                                    <Badge bg={`${myUser.isAdmin? 'info' : 'secondary'}`}>{myUser.isAdmin? "ADMIN" : myUser.department}</Badge>
                                                </div>
                                                <p>{myUser?.email}</p>
                                            </div>
                                        </div>
                    ...
                    <div className="modal-footer">
                        ...
                        <button className="btn btn-secondary" data-bs-target="#taskInfoDetailsModal" data-bs-toggle="modal">Back</button>
                        <button
