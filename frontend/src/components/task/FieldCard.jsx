    ...
    return (
        <div className="card m-2 my-4">
            <div className="card-header p-2">
                <h5 className="fs-6 d-flex gap-3 align-items-center ">
                    ...
                    <p className="fw-normal">
                        {field?.ff_title &&
                        field?.ff_title.includes("Gaurantee")
                            ? "Guarantee"
                            : field?.ff_title}
                    </p>
                </h5>
            ...
            <div className="card-body">
                {field?.ff_columns?.length === 0 && (
                    <p>{field?.ff_description}</p>
                )}
                {field?.ff_columns?.map((fieldColumn, idx) => (
                    <Form.Group
                        key={`field-column-${idx}`}
                        className="mb-3"
                        controlId={`field-column-${idx}`}
                    >
                        ...
                        {!fieldColumn?.large_text == true ? (
                            fieldColumn?.type === "FILE" ? (
                                ...
                                <Form.Control
                                    ...
                                    name="value"
                                    value={
                                        newFunction?.functionFields[fieldIndex]
                                            ?.fieldColumns[idx]?.value
                                    }
                                    onChange={(e) => {
                                        console.log("changing...");
                                        handleFunctionChange(
