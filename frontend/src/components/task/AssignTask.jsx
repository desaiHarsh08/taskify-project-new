...
const AssignTask = ({ 
    task, allUsers, selectedUser, setSelectedUser
}) => {
                ...
                <div className='assign-user'>
                    ...
                    <Container fluid className='p-0 my-3  overflow-auto' style={{ height: "350px" }}>
                        ...
                        {usersArr.map((user, index) => (
                            <div
                                className={`d-flex gap-4 ${selectedUser === user.id ? 'bg-light' : ''} align-items-center p-2 border-bottom`}
                            >
                                <input
                                    type="radio"
                                    name="assignee"
                                ...
                                <div>
                                    <div className='d-flex gap-2'>
                                        <p>{user?.name}</p>
                                        <Badge bg={`${user.isAdmin ? 'info' : 'secondary'}`}>{user.isAdmin ? "ADMIN" : user.department}</Badge>
                                    </div>
                                    <p>{user?.email}</p>
                                </div>
                            </div>
